// Vercel Serverless Function: GET /api/chain
function norm(x) {
...
export default function handler(req, res) {
  const underlying = (req.query?.underlying || 'NIFTY').toString();
  const expiry = (req.query?.expiry || '0').toString();
  ...
  for (let i = -6; i <= 6; i++) {
    ...
    rows.push({
      strike,
      atm: i === 0,
      callLTP: Number(call.price.toFixed(1)),
      ...
      callIV: call.iv,
      callOI,
      putLTP: Number(put.price.toFixed(1)),
      putDelta: put.delta,
      putGamma: put.gamma,
      putTheta: put.theta,
      putVega: put.vega,
  ...
  res.status(200).json({
    ...
    vixChange: -0.54,
    vixChangePct: -2.5,
    futPrice: Number((spot * 1.0015).toFixed(2)),
    rows,
  });
}
