// Vercel Serverless Function: POST /api/backtest
export default function handler(req, res) {
  ...
  const cfgs = {
    ...
    ic: {
      ...
      avgLossMul: 0.25,
      name: 'Iron Condor',
      legs: ['BUY OTM+2 CE', 'SELL OTM+1 CE', 'SELL OTM-1 PE', 'BUY OTM-2 PE'],
      premMul: 0.005,
    },
  ...
  const trades = [];
  let cumPnl = 0;
  const seed = underlying.toString().charCodeAt(0) + strategy.toString().length;
  ...
  for (let i = 0; i < totalWeeks; i++) {
    ...
    const dy = ((i * 7) % 28) + 1;
    const yr = year.toString().slice(0, 4);
    trades.push({
      week: i + 1,
  ...
  return res.status(200).json({
    ...
    maxDrawdownPct: ((maxDD / capital) * 100).toFixed(1),
    sharpe,
    avgWeeklyPnl: Math.round(totalPnl / trades.length),
    margin,
